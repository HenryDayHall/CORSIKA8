...
int main(int argc, char** argv) {
    ...
    if (argc != 3) {
        std::cerr << "usage: radio_em_shower <energy/GeV> <seed> - put seed=0 to use random seed" << std::endl;
        return 1;
    }
    ...
    create_5layer_atmosphere<EnvironmentInterface, MyExtraEnv>(env, AtmosphereId::LinsleyUSStd, center,
...
// ----------------------- Radio objects --------------------------------------------------------------------
    ...
    for (auto radius_1 = 25_m; radius_1 <= 500_m; radius_1 += 25_m) {
        for (auto phi_1 = 0; phi_1 <= 315; phi_1 += 45) {
            ...
            auto const point_1 {Point(rootCS, showerCoreX_ + radius_1 * cos(phiRad_1), showerCoreY_ + radius_1 * sin(phiRad_1), constants::EarthRadius::Mean)};
            ...
            TimeDomainAntenna antenna_1(name_1, point_1, triggertime_1, duration_, sampleRate_, triggertime_1);
...
// primary particle times -> t ground
    ...
    ParticleCut<SubWriter<decltype(dEdX)>> cut(5_MeV, 5_MeV, 100_GeV, 100_GeV, true,
                                               dEdX);
    ...
    auto sequence =
            make_sequence(emCascade, emContinuous, longprof, cut, coreas, zhs, observationLevel, tracks);
