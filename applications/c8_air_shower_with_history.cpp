...
#include <corsika/modules/writers/HistoryWriter.hpp>
...
// for ICRC2023
  ...
  // make and register the first history writer
  HistoryWriter<setup::Tracking, ParticleWriterParquet> history_writer(
      showerAxis, observationLevel);
  output.add("history", history_writer);
  ...
  for (int i_shower = 1; i_shower < nevent + 1; i_shower++) {
    ...
    auto sequence =
        make_sequence(stackInspect, neutrinoPrimaryPythia, hadronSequence, decaySequence,
                      ...
                      observationLevel, inter_writer, history_writer, thinning, cut);
