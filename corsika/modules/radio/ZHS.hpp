...
namespace corsika {
    ...
    public:
        ...
        ProcessReturn simulate(Particle& particle, Track const& track) const {
            ...
            auto constants{charge / (4 * M_PI) / (constants::epsilonZero) / constants::c};
            ...
            for (auto& antenna : antennas_.getAntennas()) {
                ...
                for (size_t i{0}; i < midPaths.size(); i++) {
                    ...
                    double const fraunhLimit{sinTheta2 * trackLength * trackLength/ midPaths[i].R_distance_ / lambda*2*M_PI};
                    ...
                    if (fraunhLimit > 1.0)
                    {
                        ...
                        for (int j{0}; j < nSubTracks; j++){
                            ...
                            auto newMidPaths{this->propagator_.propagate(newMidPoint, antenna.getLocation(), 1_m)};
                            ...
                            for (size_t k{0}; k < newMidPaths.size(); k++){
                                ...
                                TimeType detectionTime1{time1 + newMidPaths[k].propagation_time_ - n_source*betaTimesK*(time1 - midTime)};
                                TimeType detectionTime2{time2 + newMidPaths[k].propagation_time_ - n_source*betaTimesK*(time2 - midTime)};
                                ...
                                if (detectionTime1 > detectionTime2) {
                                    detectionTime1 = time2 + newMidPaths[k].propagation_time_ - n_source*betaTimesK*(time2 - midTime);
                                    detectionTime2 = time1 + newMidPaths[k].propagation_time_ - n_source*betaTimesK*(time1 - midTime);
                                    constants = -constants;
                                }

                                double const startBin{std::floor((detectionTime1-antenna.start_time_) * antenna.sample_rate_ + 0.5L)};
                                double const endBin{std::floor((detectionTime2-antenna.start_time_) * antenna.sample_rate_ + 0.5L)};

                                auto const betaPerp{newMidPaths[k].emit_.cross(beta.cross(newMidPaths[k].emit_))};
                                ...
                                if (startBin == endBin) {
                                    ...
                                    if (std::fabs(denominator) > 1.e-15L) {
                                        ...
                                        VectorPotential const Vp = betaPerp / denominator / newMidPaths[k].R_distance_ * constants * f;
                                        antenna.receive(detectionTime2, betaPerp, Vp);
                                    } else { // If emission in Cerenkov angle => approximation
                                        double const f{(detectionTime2 - detectionTime1) *
                                                       antenna.sample_rate_};
                                        VectorPotential const Vp = betaPerp / newMidPaths[k].R_distance_ * constants * f;
                                        antenna.receive(detectionTime2, betaPerp, Vp);
                                    ...
                                    double f{std::fabs(startBin + 0.5L - (detectionTime1-antenna.start_time_) * antenna.sample_rate_)};
                                    VectorPotential Vp = betaPerp * f * constants / denominator / newMidPaths[k].R_distance_;
                                    ...
                                    f = std::fabs((detectionTime2-antenna.start_time_) * antenna.sample_rate_ + 0.5L - endBin);
                                    Vp = betaPerp * f * constants / denominator / newMidPaths[k].R_distance_;
                                    antenna.receive(detectionTime2, betaPerp, Vp);
                    ...
                    } else // Calculate vector potential of whole track
                    {
                        ...
                        TimeType detectionTime1{startTime + midPaths[i].propagation_time_ - n_source*betaTimesK*(startTime - midTime)};
                        TimeType detectionTime2{endTime + midPaths[i].propagation_time_ - n_source*betaTimesK*(endTime - midTime)};

                        // make detectionTime1_ be the smallest time => changes step function order so
                        // constants is changed to account for it
                        if (detectionTime1 > detectionTime2) {
                            detectionTime1 = endTime + midPaths[i].propagation_time_ - n_source*betaTimesK*(endTime - midTime);
                            detectionTime2 = startTime + midPaths[i].propagation_time_ - n_source*betaTimesK*(startTime - midTime);
                            constants = -constants;
                        }
                        ...
                        double const denominator{1. - midPaths[i].refractive_index_source_ * betaTimesK};
                        ...
                        if (startBin == endBin) {
                            ...
                            if (std::fabs(denominator) > 1.e-15L) {
                                ...
                                VectorPotential const Vp = betaPerp / denominator / midPaths[i].R_distance_ * constants * f;
                                antenna.receive(detectionTime2, betaPerp, Vp);
                            } else { // If emission in Cerenkov angle => approximation
                                double const f{(detectionTime2 - detectionTime1) *
                                               antenna.sample_rate_};
                                VectorPotential const Vp = betaPerp / midPaths[i].R_distance_ * constants * f;
                                antenna.receive(detectionTime2, betaPerp, Vp);
                            } // end if Cerenkov angle approx
                        } else {
                            /*Track is contained in more than one bin*/
                            ...
                            // first contribution
                            double f{std::fabs(startBin + 0.5L - (detectionTime1-antenna.start_time_) * antenna.sample_rate_)};
                            VectorPotential Vp = betaPerp * f * constants / denominator / midPaths[i].R_distance_;
                            antenna.receive(detectionTime1, betaPerp, Vp);
                            // intermidiate contributions
                            for (int it{1}; it < numberOfBins; ++it) {
                                Vp = betaPerp * constants / denominator / midPaths[i].R_distance_;
                                antenna.receive(
                                        detectionTime1 + static_cast<double>(it) / antenna.sample_rate_,
                                        betaPerp, Vp);
                            } // end loop over bins in which potential vector is not zero
                            // final contribution
                            f = std::fabs((detectionTime2-antenna.start_time_) * antenna.sample_rate_ + 0.5L - endBin);
                            Vp = betaPerp * f * constants / denominator / midPaths[i].R_distance_;
                            antenna.receive(detectionTime2, betaPerp, Vp);
                        } // end if statement for track in multiple bins
