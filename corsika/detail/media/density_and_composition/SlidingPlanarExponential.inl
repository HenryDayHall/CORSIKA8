...
namespace corsika {
  namespace media {
    ...
    inline SlidingPlanarExponential<TDerived>::SlidingPlanarExponential(
        ...
        media::NuclearComposition const& nuclComp, LengthType const referenceHeight)
    ...
    inline LengthType SlidingPlanarExponential<TDerived>::getArclengthFromGrammage(
      ...
      auto const axis =
          (traj.getPosition(0) -
           BaseExponential<SlidingPlanarExponential<TDerived>>::getAnchorPoint())
              .normalized();
      return BaseExponential<
          SlidingPlanarExponential<TDerived>>::getArclengthFromGrammage(traj, grammage,
                                                                        axis);
    }
