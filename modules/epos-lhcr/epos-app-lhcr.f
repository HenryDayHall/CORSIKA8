...
9999  continue
     ...
     &       ,'  events for engy =',engy,'  ...'
            ...
            if(istore.gt.0.and.iappl.ne.3.) call bstora
            do  n=1,nevent
              ...
              enddo
 77           continue
c  77          if(ispherio.ne.0)then
c                if(nfr.eq.0)write(ifmt,'(a)')
c     &           'spherio evolution + hadronization ...'
c                if(mod(nfr+1,50).eq.0)
c     &          write(ifmt,*)'hadronization ',nfr+1,' / ',nfreeze
cc                  call spherio2(nrevt,nfr)
c                if(ish.ge.2)call alist('list after spherio&',1,nptl)
c                call decayall(1)
c                if(ish.ge.2)call alist('list after decay&',1,nptl)
c             endif
              if(abs(ninicon).ne.1)then
                call aafinal
...
C-----------------------------------------------------------------------
      ...
      DOUBLE PRECISION A,B,RR,RD(2),U1,U2,FAC
      logical KNOR
      SAVE
C-----------------------------------------------------------------------
